//
// PERQToZ80FIFO.cs - Copyright (c) 2006-2025 Josh Dersch (derschjo@gmail.com)
//
    ...
    public class PERQToZ80FIFO : IZ80Device
    {
        ...
        public void Enqueue(int value)
        {
            ...
            lock (_lock)
            {
                if (_fifo.Count < 16)
                {
                    ...
                    Log.Detail(Category.FIFO, "PERQ wrote byte 0x{0:x2} ({1} bytes)", value, _fifo.Count);
                }
                else
                {
        ...
        public byte Read(byte portAddress)
        {
            ...
            lock (_lock)
            {
                ...
                {
                    value = _fifo.Dequeue();
                    Log.Detail(Category.FIFO, "Z80 read byte 0x{0:x2} ({1} bytes left)", value, _fifo.Count);
                }
                else
                {
